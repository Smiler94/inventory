...
Calendar.language = {
	...
	"date2StringPattern" : ["yyyy-MM-dd hh:mm:ss", "yyyy-MM-dd hh:mm:ss", "yyyy-MM-dd hh:mm:ss", "yyyy-MM-dd hh:mm:ss", "yyyy-MM-dd hh:mm:ss"],//增加时分秒
	"string2DatePattern" : ["ymdhms","ymdhms", "ymdhms", "ymdhms", "ymdhms"]
};
...
Calendar.prototype.draw = function() {
	...
	//新增时分秒
	_cs[_cs.length] = ' <tr><th colspan="2">';
	_cs[_cs.length] = "\u65f6<select name=\"select_hour\" id=\"select_hour\">";
	for (h = 0; h <= 9; h++) {
		_cs[_cs.length] = "<option value=\"0" + h + "\">0" + h + "</option>";
	}
	for (h = 10; h <= 23; h++) {
		_cs[_cs.length] = "<option value=\"" + h + "\">" + h + "</option>";
	}
	_cs[_cs.length] = "</select></th><th colspan=\"2\"> \u5206<select name=\"select_minute\" id=\"select_minute\">";
	for (m = 0; m <= 9; m++) {
		_cs[_cs.length] = "<option value=\"0" + m + "\">0" + m + "</option>";
	}
	for (m = 10; m <= 59; m++) {
		_cs[_cs.length] = "<option value=\"" + m + "\">" + m + "</option>";
	}
	_cs[_cs.length] = "</select></th><th colspan=\"2\"> \u79d2<select name=\"select_second\" id=\"select_second\">";
	for (s = 0; s <= 9; s++) {
		_cs[_cs.length] = "<option value=\"0" + s + "\">0" + s + "</option>";
	}
	for (s = 10; s <= 59; s++) {
		_cs[_cs.length] = "<option value=\"" + s + "\">" + s + "</option>";
	}
		_cs[_cs.length] = '</th><\/tr>';
	//新增时分秒
	_cs[_cs.length] = ' <tr>';
			...
			tds[i].onclick = function () {
				if (calendar.dateControl){
					calendar.dateControl.value = new Date(calendar.date.getFullYear(),
														...
														this.innerHTML,__calendarIframe.document.getElementById("select_hour").value,__calendarIframe.document.getElementById("select_minute").value,__calendarIframe.document.getElementById("select_second").value).format(calendar.date2StringPattern);
				}
				//alert(calendar.dateControl.value);
				//alert(this.innerHTML+calendar.date2StringPattern);
				calendar.hide();
			}
...
var divs = [];
divs[divs.length] = '<div id="__calendarPanel" style="position:absolute;display:none;background-color:#FFFFFF;border:1px solid #666666;width:200px;height:270px; z-index: 600;">';
